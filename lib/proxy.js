...
module.exports = function(fnLog) {
	var rProxyUrl = /^\/(http|https)\/([^/]+)\/?(.*)/;
	fnLog = fnLog || console.log;
	...
	return function (oRequest, oResponse) {
		...
		fnLog('Proxy request incoming for protocol: '  + sProtocol + ' host: ' + sHost + ' path: ' + oRequest.url);
		...
		oProxyServer.on('proxyReq', function(oProxyReq, oRequest) {
			fnLog('Proxy request url: ' + oRequest.url);
