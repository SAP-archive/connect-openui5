...
var themeFilesPattern = /\/library(\.css|-RTL\.css|-parameters\.json)$/;
...
module.exports = function(options) {
	...
	function findInPaths(filePath, cb) {
		...
		async.concat(options.paths.map(function(dirPath) {
	...
	function compileLess(themeDir, lessInfo, suffix, res, next) {
		fs.readFile(lessInfo.path, {
			...
			less.build(str, {
				...
				async.map(result.imports, fs.stat, function(err, importStats) {
					if (err) {
						next(err);
						return;
					}
					...
					serveFromCache(themeDir, suffix, res, next);
	...
	function serveFromCache(themeDir, suffix, res, next) {
		...
		switch (suffix) {
			...
			case '-parameters.json':
				...
				res.write(JSON.stringify(theme.variables, null, 4));
				break;
			// no default
		}
		res.end();
	...
	return function lessMiddleware(req, res, next) {
		...
		findInPaths(dirname + '/library.source.less', function(err, results) {
					...
					async.map(themeCache.imports.paths, function(filePath, cb) {
						...
						for (var i = 0; i < newImportStats.length; i++) {
							...
							if (!newImportStats[i] || (newImportStats[i].mtime.getTime() !== themeCache.imports.stats[i].mtime.getTime())) {
								compileLess(dirname, lessInfo, match[1], res, next);
								return;
							}
						}

						// use cached content if no file was changed
						serveFromCache(dirname, match[1], res, next);
